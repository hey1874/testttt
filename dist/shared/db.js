import { cloneDeep, createId, normalizeSpecs, normalizeText, parseTags, slugify, sortCategories, sortProducts } from './utils.js';
...
function normalizeDatabase(raw) {
    ...
    const categories = sortCategories((Array.isArray(source.categories) ? source.categories : FALLBACK_CATEGORIES).map(normalizeCategory));
    const products = sortProducts((Array.isArray(source.products) ? source.products : []).map((product, index)=>normalizeProduct(product, index, categories)));
...
export class JsonDatabase {
    ...
    getCategory(categoryId) {
        this.assertLoaded();
        return cloneDeep(this.snapshot.categories.find((category)=>category.id === categoryId));
    }
    getProduct(productId) {
        this.assertLoaded();
        return cloneDeep(this.snapshot.products.find((product)=>product.id === productId));
    }
    saveCategory(draft) {
        this.assertLoaded();
        ...
        this.snapshot.categories = sortCategories(categories);
        this.commit();
    ...
    resetToSeed() {
        this.assertLoaded();
        this.snapshot = cloneDeep(this.seed);
        this.commit();
        return this.getSnapshot();
    }
    subscribe(listener) {
