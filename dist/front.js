...
import { escapeHtml, formatCurrency, formatDateTime, getCategoryMap, getProductImage, normalizeText } from './shared/utils.js';
...
function persistUiState() {
    try {
        localStorage.setItem(FRONT_STATE_KEY, JSON.stringify({
            budget: state.budget,
...
function ensureState() {
    ...
    const categoryExists = state.activeCategoryId === 'all' || snapshot.categories.some((category)=>category.id === state.activeCategoryId);
    if (!categoryExists) {
        state.activeCategoryId = 'all';
    }
    state.selectedProductIds = state.selectedProductIds.filter((productId)=>snapshot.products.some((product)=>product.id === productId));
...
function getSelectedProducts() {
    const snapshot = getSnapshot();
    return state.selectedProductIds.map((productId)=>snapshot.products.find((product)=>product.id === productId)).filter(Boolean);
}
...
function toggleSelection(productId) {
    ...
    state.selectedProductIds = exists ? state.selectedProductIds.filter((item)=>item !== productId) : [
...
function renderCategoryTabs() {
    ...
    const snapshot = getSnapshot();
    const categoriesHtml = snapshot.categories.map((category)=>{
...
function renderHotList() {
    ...
    const hotProducts = snapshot.products.filter((product)=>product.hot).filter((product)=>state.activeCategoryId === 'all' || product.categoryId === state.activeCategoryId).slice(0, 3);
    if (hotProducts.length === 0) {
        elements.hotList.innerHTML = '<div class="empty-state">当前分类还没有热门推荐，可以去后台把商品标记为热门。</div>';
        return;
    }
    elements.hotList.innerHTML = hotProducts.map((product)=>{
...
function renderSelection() {
  ...
  `;
    ...
    elements.selectedList.innerHTML = products.map((product)=>{
        const category = categoryMap.get(product.categoryId);
        return `
...
function renderDialog() {
    ...
    <div class="spec-grid">
      ${buildDisplaySpecs(product).map((spec)=>`
