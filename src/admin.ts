import type { Category, CategoryDraft, Database, Product, ProductDraft, ProductSpecDraft } from './shared/types.ts';
import { JsonDatabase } from './shared/db.ts';
import { escapeHtml, formatCurrency, formatDateTime, getCategoryMap, normalizeText, parsePrice, tagsToText } from './shared/utils.ts';
...
const elements = {
  ...
  categoryForm: document.getElementById('category-form') as HTMLFormElement | null,
  categoryName: document.getElementById('category-name') as HTMLInputElement | null,
  categoryCode: document.getElementById('category-code') as HTMLInputElement | null,
  categoryAccent: document.getElementById('category-accent') as HTMLInputElement | null,
  categoryOrder: document.getElementById('category-order') as HTMLInputElement | null,
  categoryDescription: document.getElementById('category-description') as HTMLTextAreaElement | null,
  ...
  adminSearch: document.getElementById('admin-search') as HTMLInputElement | null,
  newProductBtn: document.getElementById('new-product-btn'),
  productTableBody: document.getElementById('product-table-body'),
  productForm: document.getElementById('product-form') as HTMLFormElement | null,
  productFormTitle: document.getElementById('product-form-title'),
  productCategory: document.getElementById('product-category') as HTMLSelectElement | null,
  productName: document.getElementById('product-name') as HTMLInputElement | null,
  productBrand: document.getElementById('product-brand') as HTMLInputElement | null,
  productModelId: document.getElementById('product-model-id') as HTMLInputElement | null,
  productHp: document.getElementById('product-hp') as HTMLInputElement | null,
  productGuidePrice: document.getElementById('product-guide-price') as HTMLInputElement | null,
  productOfficialPrice: document.getElementById('product-official-price') as HTMLInputElement | null,
  productHot: document.getElementById('product-hot') as HTMLInputElement | null,
  productSummary: document.getElementById('product-summary') as HTMLTextAreaElement | null,
  productTags: document.getElementById('product-tags') as HTMLTextAreaElement | null,
  productImage: document.getElementById('product-image') as HTMLInputElement | null,
  ...
  importInput: document.getElementById('import-json-input') as HTMLInputElement | null,
...
function setSaveStatus(message: string, mode: 'neutral' | 'success' | 'danger' = 'neutral'): void {
...
function nextCategoryOrder(): number {
...
function ensureState(): void {
  ...
  if (
    ...
    !snapshot.categories.some((category) => category.id === state.filterCategoryId)
  ) {
    state.filterCategoryId = 'all';
  }
  ...
  if (
    ...
    !snapshot.categories.some((category) => category.id === state.editingCategoryId)
  ) {
    clearCategoryForm();
  }
  ...
  if (
    ...
    !snapshot.products.some((product) => product.id === state.editingProductId)
  ) {
    clearProductForm();
  }
...
function renderCategoryList(): void {
  ...
  const snapshot = getSnapshot();
  elements.categoryList.innerHTML = snapshot.categories
    .map((category) => {
...
function renderCategoryOptions(): void {
  const snapshot = getSnapshot();
  const options = snapshot.categories
    .map(
      (category) =>
        `<option value="${escapeHtml(category.id)}">${escapeHtml(category.name)} (${escapeHtml(category.code)})</option>`,
    )
    .join('');
  ...
  if (elements.productCategory) {
    ...
    elements.productCategory.value =
      currentValue && snapshot.categories.some((category) => category.id === currentValue)
        ? currentValue
...
function renderSpecRows(): void {
  ...
  elements.specList.innerHTML = state.productSpecs
    .map(
      (spec, index) => `
        <div class="spec-row">
...
function clearCategoryForm(): void {
...
function fillCategoryForm(category: Category): void {
...
function clearProductForm(preferredCategoryId?: string): void {
  const snapshot = getSnapshot();
  ...
  if (elements.productCategory) {
    elements.productCategory.value =
      preferredCategoryId ||
      (state.filterCategoryId !== 'all' ? state.filterCategoryId : snapshot.categories[0]?.id || '');
  }
...
function fillProductForm(product: Product): void {
  ...
  state.productSpecs = product.specs.map((spec) => ({ ...spec }));
...
function readCategoryDraft(): CategoryDraft {
  return {
...
function readProductDraft(): ProductDraft {
  return {
    ...
    tags: normalizeText(elements.productTags?.value)
      .split(/[|,，\n]/)
      .map((item) => item.trim())
      .filter(Boolean),
...
function bindEvents(): void {
  ...
  elements.categoryFilter?.addEventListener('change', (event) => {
    state.filterCategoryId = (event.currentTarget as HTMLSelectElement).value;
    renderProductTable();
  });
  ...
  elements.productTableBody?.addEventListener('click', (event) => {
    const target = event.target as HTMLElement;
    const row = target.closest<HTMLTableRowElement>('[data-product-id]');
  ...
  elements.specList?.addEventListener('click', (event) => {
    const target = event.target as HTMLElement;
    const button = target.closest<HTMLButtonElement>('[data-remove-spec]');
